...
#ifndef URBANCZIK_CONNECTION_TD_H
#define URBANCZIK_CONNECTION_TD_H
...
class UrbanczikConnectionTD : public Connection< targetidentifierT >
{
...
public:
   ...
   */
  UrbanczikConnectionTD( const UrbanczikConnectionTD& );
  ...
  static bool requires_time_driven_update() { return true; }

  void time_driven_update( Time syn_update_stamp, const thread tid,
...
template < typename targetidentifierT >
inline void
UrbanczikConnectionTD< targetidentifierT >::send( Event& e, thread t,
    const CommonSynapseProperties& cp )
{
  double t_spike = e.get_stamp().get_ms();
  time_driven_update( e.get_stamp(), t, cp );
  // use accessor functions (inherited from Connection< >) to obtain delay and target
  Node* target = get_target( t );
  double const tau_L = target->get_tau_L( 1 );
  double const tau_s = weight_ > 0.0 ? target->get_tau_syn_ex( 1 ) : target->get_tau_syn_in( 1 );
...
template < typename targetidentifierT >
inline void
UrbanczikConnectionTD< targetidentifierT >::time_driven_update( Time syn_update_stamp,
    const thread tid, const CommonSynapseProperties& cp)
{
  double t_update = syn_update_stamp.get_ms();
  ...
  target->get_urbanczik_history( t_lastupdate_ - dendritic_delay, t_update - dendritic_delay, &start, &finish, comp );
  ...
  while ( start != finish )
  {
    double const time_pp = start->t_ + dendritic_delay;
    double const time_down = time_pp - t_update;
    double const time_up = t_lastspike_ - time_pp;
    double const PSP_mem_tr =
      ( tau_L_trace_ * exp( time_up / tau_L ) - tau_s_trace_ * exp( time_up / tau_s ) ) * start->dw_;
    PI_integral_ += PSP_mem_tr;
    dPI_exp_integral += PSP_mem_tr * exp( time_down / tau_Delta_ );
    start++;
  }
  PI_exp_integral_ = ( exp( ( t_lastupdate_ - t_update ) / tau_Delta_ ) * PI_exp_integral_ +
    dPI_exp_integral );
  ...
  t_lastupdate_ = t_update;
}

template < typename targetidentifierT >
UrbanczikConnectionTD< targetidentifierT >::UrbanczikConnectionTD()
  : ConnectionBase()
  , weight_( 1.0 )
  , tau_Delta_( 100.0 )
  , eta_( 0.07 )
...
template < typename targetidentifierT >
UrbanczikConnectionTD< targetidentifierT >::UrbanczikConnectionTD( const UrbanczikConnectionTD< targetidentifierT >& rhs )
  : ConnectionBase( rhs )
  , weight_( rhs.weight_ )
...
template < typename targetidentifierT >
void
UrbanczikConnectionTD< targetidentifierT >::get_status( DictionaryDatum& d ) const
{
...
template < typename targetidentifierT >
void
UrbanczikConnectionTD< targetidentifierT >::set_status( const DictionaryDatum& d, ConnectorModel& cm )
{
...
#endif // of #ifndef URBANCZIK_CONNECTION_TD_H
