...
#ifndef CLOPATH_CONNECTION_TD_H
#define CLOPATH_CONNECTION_TD_H
...
class ClopathConnectionTD : public Connection< targetidentifierT >
{
...
public:
   ...
   */
  ClopathConnectionTD( const ClopathConnectionTD& );
  ...
  void time_driven_update( const long t_up_steps, const thread tid,
...
template < typename targetidentifierT >
inline void
ClopathConnectionTD< targetidentifierT >::send( Event& e, thread t, const CommonSynapseProperties& )
{
  ...
  //std::cout << "  depr: " << weight_ - w1 << "  w: " << weight_ << "  x_bar: " << x_bar_ << std::endl;
}

template < typename targetidentifierT >
inline void
ClopathConnectionTD< targetidentifierT >::time_driven_update( const long t_up_steps,
    const thread tid, const CommonSynapseProperties& cp)
{
  //std::cout << "trigger time driven update" << std::endl;
  double t_update = t_up_steps * Time::get_resolution().get_ms();
  ...
  target->get_LTP_history( t_lastupdate_ - dendritic_delay, t_update - dendritic_delay, &start, &finish );
  if ( start != finish )
  {
    //std::cout << "LTD hist from, to : " << start->t_ << "  " << finish->t_ << std::endl << "dts: ";
  }
...
template < typename targetidentifierT >
ClopathConnectionTD< targetidentifierT >::ClopathConnectionTD()
  : ConnectionBase()
  , weight_( 1.0 )
  , x_bar_( 0.0 )
  , tau_x_( 15.0 )
...
template < typename targetidentifierT >
ClopathConnectionTD< targetidentifierT >::ClopathConnectionTD( const ClopathConnectionTD< targetidentifierT >& rhs )
  : ConnectionBase( rhs )
  , weight_( rhs.weight_ )
  , x_bar_( rhs.x_bar_ )
  , tau_x_( rhs.tau_x_ )
...
template < typename targetidentifierT >
void
ClopathConnectionTD< targetidentifierT >::get_status( DictionaryDatum& d ) const
{
...
template < typename targetidentifierT >
void
ClopathConnectionTD< targetidentifierT >::set_status( const DictionaryDatum& d, ConnectorModel& cm )
{
