...
#ifndef AEIF_PSC_delta_eprop_H
#define AEIF_PSC_delta_eprop_H
 ...
 */
extern "C" int
aeif_psc_delta_eprop_dynamics( double, const double*, double*, void* );
...
Name: aeif_psc_delta_eprop - Current-based adaptive exponential integrate-and-fire
neuron model according to Brette and Gerstner (2005) with delta synapse.

Description:

aeif_psc_delta_eprop is the adaptive exponential integrate and fire neuron
...
*/
class aeif_psc_delta_eprop : public Eprop_Archiving_Node
{

public:
  aeif_psc_delta_eprop();
  aeif_psc_delta_eprop( const aeif_psc_delta_eprop& );
  ~aeif_psc_delta_eprop();
...
private:
  ...
  friend int aeif_psc_delta_eprop_dynamics( double, const double*, double*, void* );

  // The next two classes need to be friends to access the State_ class/member
  friend class RecordablesMap< aeif_psc_delta_eprop >;
  friend class UniversalDataLogger< aeif_psc_delta_eprop >;
...
public:
  ...
  struct Buffers_
  {
    Buffers_( aeif_psc_delta_eprop& );                  //!<Sets buffer pointers to 0
    Buffers_( const Buffers_&, aeif_psc_delta_eprop& ); //!<Sets buffer pointers to 0

    //! Logger for all analog data
    UniversalDataLogger< aeif_psc_delta_eprop > logger_;
  ...
  static RecordablesMap< aeif_psc_delta_eprop > recordablesMap_;
};

inline port
aeif_psc_delta_eprop::send_test_event( Node& target,
  rport receptor_type,
  synindex,
  bool )
{
...
inline port
aeif_psc_delta_eprop::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
aeif_psc_delta_eprop::handles_test_event( LearningSignalConnectionEvent&,
  rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
aeif_psc_delta_eprop::handles_test_event( DataLoggingRequest& dlr,
  rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline void
aeif_psc_delta_eprop::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d );
...
inline void
aeif_psc_delta_eprop::set_status( const DictionaryDatum& d )
{
