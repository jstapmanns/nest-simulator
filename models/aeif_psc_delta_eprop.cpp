...
nest::RecordablesMap< nest::aeif_psc_delta_eprop >
  nest::aeif_psc_delta_eprop::recordablesMap_;
...
template <>
void
RecordablesMap< aeif_psc_delta_eprop >::create()
{
  // use standard names whereever you can for consistency!
  insert_(
    names::V_m, &aeif_psc_delta_eprop::get_y_elem_< aeif_psc_delta_eprop::State_::V_M > );
  insert_(
    names::w, &aeif_psc_delta_eprop::get_y_elem_< aeif_psc_delta_eprop::State_::W > );
}
}


extern "C" int
nest::aeif_psc_delta_eprop_dynamics( double,
  const double y[],
  double f[],
  void* pnode )
{
  // a shorthand
  typedef nest::aeif_psc_delta_eprop::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const nest::aeif_psc_delta_eprop& node =
    *( reinterpret_cast< nest::aeif_psc_delta_eprop* >( pnode ) );
...
nest::aeif_psc_delta_eprop::Parameters_::Parameters_()
  : V_peak_( 0.0 )    // mV
  , V_reset_( -60.0 ) // mV
  , t_ref_( 0.0 )     // ms
  , g_L( 30.0 )       // nS
...
nest::aeif_psc_delta_eprop::State_::State_( const Parameters_& p )
  : r_( 0 )
{
  y_[ 0 ] = p.E_L;
...
nest::aeif_psc_delta_eprop::State_::State_( const State_& s )
  : r_( s.r_ )
{
...
nest::aeif_psc_delta_eprop::State_& nest::aeif_psc_delta_eprop::State_::operator=(
  const State_& s )
{
...
void
nest::aeif_psc_delta_eprop::Parameters_::get( DictionaryDatum& d ) const
{
...
void
nest::aeif_psc_delta_eprop::Parameters_::set( const DictionaryDatum& d )
{
...
void
nest::aeif_psc_delta_eprop::State_::get( DictionaryDatum& d ) const
{
...
void
nest::aeif_psc_delta_eprop::State_::set( const DictionaryDatum& d,
  const Parameters_& )
{
...
nest::aeif_psc_delta_eprop::Buffers_::Buffers_( aeif_psc_delta_eprop& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::aeif_psc_delta_eprop::Buffers_::Buffers_( const Buffers_&, aeif_psc_delta_eprop& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::aeif_psc_delta_eprop::aeif_psc_delta_eprop()
...
nest::aeif_psc_delta_eprop::aeif_psc_delta_eprop( const aeif_psc_delta_eprop& n )
...
void
nest::aeif_psc_delta_eprop::init_state_( const Node& proto )
{
  const aeif_psc_delta_eprop& pr = downcast< aeif_psc_delta_eprop >( proto );
  S_ = pr.S_;
}

void
nest::aeif_psc_delta_eprop::init_buffers_()
{
  ...
  B_.sys_.function = aeif_psc_delta_eprop_dynamics;

  B_.I_stim_ = 0.0;
}

void
nest::aeif_psc_delta_eprop::calibrate()
{
...
void
nest::aeif_psc_delta_eprop::update( const Time& origin,
  const long from,
  const long to )
{
  assert(
...
void
nest::aeif_psc_delta_eprop::handle( SpikeEvent& e )
{
...
void
nest::aeif_psc_delta_eprop::handle( CurrentEvent& e )
{
  assert( e.get_delay_steps() > 0 );
...
void
nest::aeif_psc_delta_eprop::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
